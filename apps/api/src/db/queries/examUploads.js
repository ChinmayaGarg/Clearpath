 ...
 */
export async function createUpload(schema, {
  professorProfileId, courseCode, examTypeLabel, versionLabel,
  delivery, materials, password, rwgFlag, isMakeup, makeupNotes,
}) {
  const result = await tenantQuery(schema,
    `INSERT INTO exam_upload
     ...
     RETURNING id`,
    [
      professorProfileId, courseCode, examTypeLabel, versionLabel ?? null,
      delivery ?? 'pending', materials ?? null, password ?? null,
      rwgFlag ?? false, isMakeup ?? false, makeupNotes ?? null,
    ]
  );
 ...
 */
export async function updateUpload(schema, uploadId, professorProfileId, fields) {
  const allowed = [
    'course_code', 'exam_type_label', 'version_label', 'delivery',
    'materials', 'password', 'rwg_flag', 'is_makeup', 'makeup_notes',
  ];
  const setClauses = [];
  const values     = [];
  let   idx        = 1;
 ...
 */
export async function respondToReuseRequest(schema, requestId, {
  ...
  return tenantTransaction(schema, async (client) => {
    ...
    if (status === 'approved') {
      await client.query(
        ...
        [original_upload_id, makeup_exam_id]
      );
    }
  });
}
