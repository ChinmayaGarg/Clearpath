    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center gap-6">
          <Link to="/" className="font-semibold text-brand-800">Clearpath</Link>
          <Link to="/" className="text-sm text-gray-500 hover:text-gray-900">Book</Link>
          <Link to="/professors" className="text-sm text-gray-500 hover:text-gray-900">Professors</Link>
          <Link to="/students" className="text-sm text-gray-500 hover:text-gray-900">Students</Link>
          ...
          {overview && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <StatCard
                ...
                colour={pct(overview.completed_exams, overview.total_exams) > 80 ? 'green' : 'amber'}
              />
              <StatCard
                label="Emails sent"
                ...
                {leads.map((l, i) => (
                  <div key={i} className="flex items-center justify-between
                    ...
                    <div className="flex gap-3 text-right">
                      <div>
                        <div className="text-sm font-semibold text-gray-900">{l.emails_sent}</div>
                        <div className="text-xs text-gray-400">emails</div>
                      </div>
                      <div>
                        <div className="text-sm font-semibold text-gray-900">{l.status_changes}</div>
                    ...
                    <div key={i}>
                      <div className="flex items-center justify-between mb-0.5">
                        <span className={`text-xs font-medium ${
                          a.triggers_rwg_flag ? 'text-red-700' : 'text-gray-700'
                        }`}>
                          {a.code}
                        </span>
          ...
          <Section title="Email response times" loading={loading}>
            <div className="flex gap-6">
              <div>
                ...
                <div className="text-sm text-gray-500">Average professor response time</div>
              ...
              <div>
                <BarChart
                  ...
                  colour="#534AB7"
                />
                <div className="text-xs text-gray-400 text-center mt-1">Emails sent per day</div>
              </div>
            </div>
          </Section>
        )}
        ...
        {overview?.status_breakdown && (
          <Section title="Exam status breakdown" loading={loading}>
            <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
              {[
                { key: 'pending',   label: 'Pending',   colour: 'bg-gray-100 text-gray-600'     },
                { key: 'emailed',   label: 'Emailed',   colour: 'bg-blue-100 text-blue-700'      },
                { key: 'received',  label: 'Received',  colour: 'bg-yellow-100 text-yellow-700'  },
                { key: 'written',   label: 'Written',   colour: 'bg-orange-100 text-orange-700'  },
                { key: 'picked_up', label: 'Picked up', colour: 'bg-green-100 text-green-700'    },
