...
export default function Professors() {
  ...
  useEffect(() => {
    ...
    if (!q) { setFiltered(professors); return; }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center gap-6">
          ...
          <span className="text-sm text-gray-900 font-medium">Professors</span>
        </div>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">Professor directory</h1>
            <p className="text-sm text-gray-500 mt-0.5">
              {professors.length} professor{professors.length !== 1 ? 's' : ''} in this institution
            </p>
          </div>
          <button
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-sm text-gray-400">
            {search ? 'No professors match your search' : 'No professors yet — add one to get started'}
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="text-center px-4 py-3 text-xs font-medium text-gray-500">Dossiers</th>
                  <th className="text-center px-4 py-3 text-xs font-medium text-gray-500">Exams</th>
                  <th className="text-left px-4 py-3 text-xs font-medium text-gray-500">Last exam</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(p => (
                  <ProfRow key={p.id} prof={p} onClick={() => setSelected(p.id)} />
                ))}
              </tbody>
            </table>
          </div>
