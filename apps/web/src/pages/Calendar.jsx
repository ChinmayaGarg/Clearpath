...
function CalCell({ date, summary, isToday, isViewing, onClick }) {
  ...
  return (
    <button onClick={onClick}
      ...
      <span className={`font-medium ${isToday ? 'text-brand-800' : 'text-gray-700'}`}>
        {d.getDate()}
      </span>
      {summary && (
      ...
      <div className="max-w-2xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <button onClick={prevMonth}
            className="p-2 rounded-lg border border-gray-200 hover:bg-gray-100">←</button>
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }, (_, i) => {
            ...
            return (
              <CalCell key={d} date={d}
                summary={dateMap[d]}
                isToday={d === today}
                isViewing={d === date}
                onClick={() => { setDate(d); navigate('/'); }}
              />
            );
          })}
        </div>
